...
for v in vendas:
    if v["id"] not in ids_vistos:
        ...
        # Tratamento de valores faltantes em quantidade (usa 1 como valor padrão)
for v in vendas_unicas:
    if v["quantidade"] is None:
        v["quantidade"] = 1  # valor padrão em caso de falha

# Tratamento de valores faltantes em preço (usa preço do catálogo) 
...
for v in vendas_unicas:
    ...
    if v["produto"] not in totais_valor:
        ...
        totais_valor[v["produto"]] += valor
        totais_qtd[v["produto"]] += v["quantidade"]
    if v["produto"] not in totais_qtd:
        totais_qtd[v["produto"]] = v["quantidade"]
    else:
        totais_qtd[v["produto"]] += v["quantidade"]
...
print(f"\nProduto com mais vendas: {campeao_valor} ({totais_valor[campeao_valor]:.2f})")
